...
export default {
  ...
  actions: {
    ...
    GET_MEMBER_INFO: async ({commit, state, dispatch}, params) => {
      ...
      if (params && params.item === 'balance') {
        ...
        let info = await dispatch('GET_MEMBER_INFO2')
        commit('setUser', {...state.userInfo, ...res.data.user, balance: info.data.balance})
      }
      return res
    },
    LOGOUT: async ({commit, state}, params) => {

...
function generateLeftTitle({ lotteryType, num, navTitle, subNavTitle }) {
  ...
  if (lotteryType === 'syxw') {
    allTitleList = ['第一球', '第二球', '第三球', '第四球', '第五球']
  }
...
// 生成和值数据
function generateSumList({ playMenu, min, max, repeat }) {
  let navTitle = playMenu.split('-')[0]
  let subNavTitle = playMenu.split('-')[1]
  let result = []
  let sumNumList = [
    {
      cn: '二',
...
export function generateBetTplData({
  ...
  if (
    ...
    result.betData.push({
      title: '和值',
      list: generateSumList({ playMenu, min, max, repeat }).map(item => {
        return {
          name: item,
          odds_min: playInfo.odds_min,
          odds_max: playInfo.odds_max
  ...
  // pk10 冠亚和玩法
  if (lotteryType === 'pk10' && navTitle === '冠亚和') {
    let numberData = betData[0].children.filter(item => !isNaN(item.name))

    let NotNumberData = betData[0].children.filter(item => isNaN(item.name))
    NotNumberData.splice(4, 0, {
      empty: true
    })
    result.betData = [
      {
        title: '选码',
        list: numberData.map(item => {
          return {
            name: item.name,
            odds_min: item.odds_min,
            odds_max: item.odds_max,
  ...
  if (
    ...
    filteredTitleList.map(item => {
      let obj = {}
      if (navTitle === '两面' && item === '总和') {
        obj.horizontal, (obj.width = 'one-fifth')
      }
      result.betData.push({
        title: item,
        list: _.cloneDeep(list),
        selectedList: [],
        ...obj,
        horizontal: lotteryType === 'ssc' && item === '总和'
      })
    })
  }
  if (lotteryType === 'xync') {
  ...
  generateEmptyCell(position,result.betData)
  return result
}

function generateEmptyCell(position,betData) {
  if(position===0){
    return betData
  }
  if(betData.length<5){
  ...
  betData.map(item => {
    let itemLen
    if (item.width === 'one-fifth' || item.title === '梭哈'||item.horizontal) {
      itemLen = 5
    }
    if (item.width === 'one-fourth') {
      itemLen = 4
    }
    if (itemLen && item.list.length % itemLen) {
